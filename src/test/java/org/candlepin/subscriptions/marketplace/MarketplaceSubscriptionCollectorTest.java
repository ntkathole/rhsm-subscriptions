...
class MarketplaceSubscriptionCollectorTest {
    ...
    @Test
    void testFetchSubscription() throws ApiException {
        when(subscriptionService.getSubscriptionsByAccountNumber(anyString()))
            .thenReturn(List.of(new Subscription()));

        UsageCalculation.Key usageKey = new UsageCalculation.Key("productId", ServiceLevel.EMPTY,
            Usage.EMPTY);

        List<Subscription> result = marketplaceSubscriptionCollector.fetchSubscription("6661312", usageKey);

        Assertions.assertEquals(List.of(new Subscription()), result);
    }
}
