...
class HttpClientTest {
  ...
  void testNoCustomTruststoreRequired() throws Exception {
    ...
    Client httpClient =
        HttpClient.buildHttpClient(x509Config, null, false);

    ProcessingException e =
        assertThrows(ProcessingException.class, () -> invokeHello(httpClient));
    ...
    assertThat(e.getCause().getMessage(), Matchers.containsString("unable to find valid certification path"));
  }
  ...
  private String invokeHello(Client client) throws IOException {
    return client.target(UriBuilder.fromUri(server.url("/hello")))
